...
async function startServer() {
  ...
  app.post("/api/v1/event-banner", async (req, res) => {
    ...
    try {
      ...
      const slugifyPath = (str: string) =>
        str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9_.-]/g, "_").toLowerCase();

      const safeName = slugifyPath(fileName || "banner.jpg");
  ...
  async function leaderMayDeleteMuralNoticeServer(zeladorId: string, noticeTenantId: string): Promise<boolean> {
    const { data: pl } = await supabaseAdmin
      .from("perfil_lider")
      .select("id, tenant_id")
      .eq("id", zeladorId)
  ...
  app.delete("/api/notices/:id", async (req, res) => {
    ...
    try {
      ...
      const allowed = await leaderMayDeleteMuralNoticeServer(zeladorId, String(notice.tenant_id || ""));
